...
public class CupsMain {
    ...
    private static void getJobs(String host, String userName, String printerName) throws Exception {
        if (host == null) {
            host = de.spqrinfo.cups4j.CupsClient.DEFAULT_HOST;
        }

        if (userName == null) {
            userName = de.spqrinfo.cups4j.CupsClient.DEFAULT_USER;
        }
        if (printerName == null) {
            de.spqrinfo.cups4j.CupsClient cupsClient = new de.spqrinfo.cups4j.CupsClient(host, de.spqrinfo.cups4j.CupsClient.DEFAULT_PORT, userName);
            printerName = cupsClient.getDefaultPrinter().getName();
        }

        de.spqrinfo.cups4j.CupsClient cupsClient = new de.spqrinfo.cups4j.CupsClient();

        // if user provided - get only jobs from this user.
        boolean myJobs = true;
        if (userName.equals(de.spqrinfo.cups4j.CupsClient.DEFAULT_USER)) {
            myJobs = false;
        }
    ...
    private static void print(String host, String printerName, String fileName, int copies, String pages, boolean duplex,
        ...
        de.spqrinfo.cups4j.CupsClient cupsClient = new de.spqrinfo.cups4j.CupsClient(host, de.spqrinfo.cups4j.CupsClient.DEFAULT_PORT);
        if (printerName == null) {

            printer = cupsClient.getDefaultPrinter();
        } else {
            printer = new CupsPrinter(new URL("http://" + host + ":" + de.spqrinfo.cups4j.CupsClient.DEFAULT_PORT + "/printers/" + printerName),
                    printerName, false);
        }
    ...
    private static void listPrintersOnHost(String hostname) throws Exception {
        ...
        while (System.currentTimeMillis() < timeoutTime && printers == null) {
            try {
                de.spqrinfo.cups4j.CupsClient cupsClient = new de.spqrinfo.cups4j.CupsClient(hostname, de.spqrinfo.cups4j.CupsClient.DEFAULT_PORT);
